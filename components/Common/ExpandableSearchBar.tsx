  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <div className="flex">
        {!isExpanded && (
          ...
          <button
            ...
            aria-label="Search"
          >
            <Search className={`h-6 w-6 stroke-2 text-primary`} />
          </button>
        )}
        ...
        <div
          className={`flex items-center bg-white transition-all duration-200 ${
            isExpanded
              ? "w-80 px-3 py-1 rounded-full border-2 border-primary transition-all hover:shadow-md shadow-lg"
              : "w-0 overflow-hidden"
          }`}
      ...
      {isExpanded && searchQuery && (
        ...
        >
          {isLoading ? (
            <div className="py-8 text-center text-primary font-medium text-sm">
              <Loading />
            </div>
